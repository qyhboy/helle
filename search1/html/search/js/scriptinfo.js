 ...
 var $li = ""; 
    ...
    var $s = document.getElementById("count");//数据要放的位置
        var $tpl_Index_List = function($p_array, $p_items) {//格式化字符串，数据集
           ...
           $li = '<tr><td>'+item.ip+'</td><td>  <a href="port.html" data-ajax="false" vbug="'+item.version+'">'+item.version+'</a></td><td><input type= "button" value="导出数据" id="exbug" exbug="'+item.ip+'"/></td><td><input type= "button" value="删除漏洞" id="debug" debug="'+item.ip+'"/></td></tr>';         
...
var list = json.list;   
  var li_array = [];
        ...
        $('#listview').on('click','a',function(){ 
              ...
              var searchinfo="http:"+$(this).attr("vbug");
        ...
        return false;
           	 ...
           	 window.location.href = "exportscript.php?action=export&bugname=" +bugname_1;
