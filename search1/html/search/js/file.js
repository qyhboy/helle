...
///////////////////////////////////////
	...
	if(str==''){
		...
		window.location.href = "sphinxget.php?action=export&port=" +port+"&version="+value;
	}
	else if(name=='version'&&port==''){
		window.location.href = "sphinxget.php?action=export&port=" +port+"&version="+value;
	}
		...
		window.location.href = "sphinxget.php?action=export&port=" +port;
     }
     else {
     	alert("您的查询不支持导出");
     }
...
$("#fileupload").click(function(){
	var name3=document.getElementById("divTip2");
    ...
    $("#fileupload").change(function(){
		$("#myupload").ajaxSubmit({
			...
			success: function(data) {
				...
				name3.innerHTML="上传成功!";
			},
			error:function(xhr){
				name3.innerHTML="上传失败!";
			}
		});
