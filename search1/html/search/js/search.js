 ...
 var $li = ""; 
    ...
    var $s = document.getElementById("count");//数据要放的位置
        var $tpl_Index_List = function($p_array, $p_items) {//格式化字符串，数据集
            ...
            $li = '<div id="ipbanner"><br /><a href="port.html" data-ajax="false" tip="'+item.ip+'"><h4>'+item.ip+'</h4></a>'; 
           $li+='<hr><div id="ipport"><a href="#"><p>'+item.os+'</p></a>';
           $li+='<div class="hostname"></div> <address ><a id="country" href="#"> <h4>'+item.address+' </h4></a><a id="city" href="#"></a> </address>';
           $li+='<p>'+item.version+'</p><p>'+item.other+'</p><p>'+item.state+','+item.states+'</p><div ><time >'+item.date+'</time></div></div>';
